...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Osternachten - Friendsystem";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Anti GommeHD - All u can do";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return "Anti GommeHD - All u can do";
#endif
}
...
const char* ts3plugin_description() {
	/* If you want to use wchar_t, see ts3plugin_name() on how to use */
    return "Osternachten - Friendsystem";
}
...
int getTalkpowerFromMyChannel(uint64 serverConnectionHandlerID)
{
	...
	uint64 mychannel;
	ts3Functions.getChannelOfClient(serverConnectionHandlerID, me, &mychannel);
	int channel_tp;
	ts3Functions.getChannelVariableAsInt(serverConnectionHandlerID, mychannel, CHANNEL_NEEDED_TALK_POWER, &channel_tp);
	return channel_tp;
}
...
{
	uint64 database_id;
	ts3Functions.getClientVariableAsUInt64(serverConnectionHandlerID, clientID, 0x20 /*CLIENT_DATABASE_ID*/, &database_id);
	uint64 channelGroupID[] = { (uint64)channelgroup };
	uint64 channelID[] = { ChannelID };
	uint64 userID[] = { database_id };
	static_assert(sizeof(channelGroupID) == sizeof(channelID), "Both ID lists have to be same size");
	ts3Functions.requestSetClientChannelGroup(serverConnectionHandlerID, channelGroupID, channelID, userID, sizeof(channelGroupID) / sizeof(channelGroupID[0]), NULL);
}
...
void createbannedusers()
{
	std::string path = "/home/osternachten/.ts3client/plugins/test.txt";
	if (exists_test3(path))
		return;
	std::ofstream outfile(path);
	outfile << "my text here2!" << std::endl;
	outfile.close();
}

std::vector<std::string> bannednames;
int loadbannedusers(char* nickname)
{
	std::string path = "/home/osternachten/.ts3client/plugins/test.txt";
	...
	std::string myString;
	myString.append(nickname);

	while (std::getline(file, str))
	{
		std::size_t found = myString.find(str);
		if (found != -1)
		{
			if (str.length() == 0)
				return 0;
...
bool found = false;
const char* uuid_search;
int freund = 0;
int freund_c = 0;

int callback(void* NotUsed, int numerOfResults, char** rowValue, char** colName);

void selectData(uint64 serverconnhandler, const char* filepath) {
	sqlite3* DB;
	sqlite3_open("", &DB);
...
int callback(void* NotUsed, int numerOfResults, char** rowValue, char** colName) {
	...
	{
		std::string a = "A: " + qq[qq.size() - 1];
		std::size_t found = qq[qq.size() - 1].find("IDS=");
		uuid_ = a;
		if (found != -1)
		{
			uuid_ = a;
	...
	//Get Friend Or Foe
	std::string friend_or_foe_;
	for (auto i = 0; i < qq.size(); i++)
	{
		std::string a = "A: " + qq[i];
		std::size_t found = qq[i].find("Friend=");
		if (found != -1)
		{
			friend_or_foe_ = a.substr(10, a.length() - 1);
			break;
		}
	}

	std::string uuid_from_client = uuid_search;
	uuid_ = uuid_.substr(7, uuid_.length());

	if (strcmp(uuid_from_client.c_str(), uuid_.c_str()) == 0)
	{
		if (atoi(friend_or_foe_.c_str()) == 0)
			freund_c = 1;
		else if (atoi(friend_or_foe_.c_str()) == 1)
			freund_c = 2;
		else if(atoi(friend_or_foe_.c_str()) == 2)
			freund_c = 0;
	}
...
void ts3plugin_onClientMoveEvent(uint64 serverConnectionHandlerID, anyID clientID, uint64 oldChannelID, uint64 newChannelID, int visibility, const char* moveMessage) {
	...
	if (clientID != me)
	{
		//ts3Functions.printMessageToCurrentTab("Ich bins nicht..");
		if (me != NULL)
		{
			//ts3Functions.printMessageToCurrentTab("Ich bin nicht NULL");
			uint64 mychannel;
			ts3Functions.getChannelOfClient(serverConnectionHandlerID, me, &mychannel);
			uint64 channel;
			ts3Functions.getChannelOfClient(serverConnectionHandlerID, clientID, &channel);
			if (mychannel == channel)
			{
				//ts3Functions.printMessageToCurrentTab("Es ist mein Channel");
				// ts3Functions.printMessageToCurrentTab("My_AnyID in meinem Channel.");
				//Alle anderen
				char* uuid;
				char* name;
				ts3Functions.getClientVariableAsString(serverConnectionHandlerID, clientID, CLIENT_NICKNAME, &name);
				ts3Functions.getClientVariableAsString(serverConnectionHandlerID, clientID, CLIENT_UNIQUE_IDENTIFIER, &uuid);
				uuid_search = uuid;
				//ts3Functions.printMessageToCurrentTab(uuid_search);
				...
				std::string ts3client;
				ts3client.append(path.u8string());
				ts3client.append("\\Ts3Client\\settings.db");
				//ts3Functions.printMessageToCurrentTab(ts3client.c_str());
				selectData(serverConnectionHandlerID, ts3client.c_str());

				if (freund_c == 1)
				{
					//ts3Functions.printMessageToCurrentTab("Freund");
					setchannelgroup(serverConnectionHandlerID, clientID, mychannel, 11);
					if (getTalkpowerFromMyChannel(serverConnectionHandlerID) != 0)
					{
						if (ts3Functions.requestClientSetIsTalker(serverConnectionHandlerID, clientID, 1, NULL) != ERROR_ok)
				...
				if (freund_c == 2)
				{
					//ts3Functions.printMessageToCurrentTab("HUAN");
					setchannelgroup(serverConnectionHandlerID, clientID, mychannel, 12);
					freund_c = 0;
					return;
				}
				//ts3Functions.printMessageToCurrentTab("NORMALER USER");
				//Ist der Name Blockiert?
				createbannedusers();
				int length = strlen(uuid);
				if (length <= 5)
					return;
				int result__ = loadbannedusers(name);
				if (result__ == 1)
				{
					setchannelgroup(serverConnectionHandlerID, clientID, mychannel, 12);
					return;
